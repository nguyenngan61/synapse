...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            user.status = 'offline'; user.lastSeen = new Date();
            io.emit('update user list', Object.values(onlineUsers));
        }
    });
    ...
    socket.on('private message', async (data) => {
        try {
            const newPm = new PrivateMessage({ fromUser: data.fromUser, toUser: data.toUser, content: data.content, type: data.type || 'text', parentMessage: data.parentMessage || null });
            await newPm.save();
    ...
    socket.on('delete private message', async (messageId) => { try { const deletedMessage = await PrivateMessage.findByIdAndDelete(messageId); if (deletedMessage) { const recipientSocketId = Object.keys(onlineUsers).find(id => onlineUsers[id].username === deletedMessage.toUser); if (recipientSocketId) { io.to(recipientSocketId).emit('private message deleted', messageId); } socket.emit('private message deleted', messageId); } } catch (error) { console.error('[SERVER] Lỗi xóa tin nhắn riêng:', error); } });
    socket.on('edit private message', async ({ messageId, newContent }) => { try { const updatedMessage = await PrivateMessage.findByIdAndUpdate(messageId, { content: newContent }, { new: true }); if (updatedMessage) { const recipientSocketId = Object.keys(onlineUsers).find(id => onlineUsers[id].username === updatedMessage.toUser); if (recipientSocketId) { io.to(recipientSocketId).emit('private message edited', updatedMessage); } socket.emit('private message edited', updatedMessage); } } catch (error) { console.error('[SERVER] Lỗi sửa tin nhắn riêng:', error); } });
