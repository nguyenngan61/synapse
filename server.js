...
mongoose
  ...
  .then(() => console.log("[SERVER] Kết nối MongoDB thành công!"))
  .catch((err) => console.error("[SERVER] Lỗi kết nối MongoDB:", err));
...
io.on("connection", (socket) => {
  socket.on("user joined", async ({ username, channel }) => {
    ...
    try {
      const messages = await Message.find({ room: channel }).sort({
        timestamp: 1,
      });
      socket.emit("load old messages", messages);
    } catch (error) {
      console.error("[SERVER] Lỗi tải tin nhắn cũ:", error);
    }
  });
  socket.on("join channel", async ({ previousChannel, newChannel }) => {
    ...
    try {
      const messages = await Message.find({ room: newChannel }).sort({
        timestamp: 1,
      });
      socket.emit("load old messages", messages);
    } catch (error) {
      console.error("[SERVER] Lỗi tải tin nhắn cho kênh mới:", error);
    }
  });
  ...
  socket.on("chat message", async (data) => {
    ...
    try {
      ...
      console.error("[SERVER] Lỗi lưu tin nhắn:", error);
    }
  });
  socket.on("private message", async (data) => {
    try {
      ...
      const recipientSocketId = Object.keys(onlineUsers).find(
        (id) => onlineUsers[id].username === data.toUser
      );
      ...
      console.error("[SERVER] Lỗi lưu tin nhắn riêng:", error);
    }
  });
  socket.on("fetch private history", async ({ user1, user2 }) => {
    try {
      ...
      console.error("[SERVER] Lỗi tải lịch sử chat riêng:", error);
    }
  });
  socket.on("delete message", async (messageId) => {
    try {
      ...
      if (deletedMessage) {
        io.to(deletedMessage.room).emit("message deleted", messageId);
      }
    } catch (error) {
      console.error("[SERVER] Lỗi xóa tin nhắn:", error);
    }
  });
  socket.on("edit message", async ({ messageId, newContent }) => {
    try {
      ...
      console.error("[SERVER] Lỗi sửa tin nhắn:", error);
    }
  });
  socket.on("delete private message", async (messageId) => {
    try {
      ...
      if (deletedMessage) {
        const recipientSocketId = Object.keys(onlineUsers).find(
          (id) => onlineUsers[id].username === deletedMessage.toUser
        );
        if (recipientSocketId) {
  ...
  socket.on("edit private message", async ({ messageId, newContent }) => {
    try {
      ...
      if (updatedMessage) {
        const recipientSocketId = Object.keys(onlineUsers).find(
          (id) => onlineUsers[id].username === updatedMessage.toUser
        );
        if (recipientSocketId) {
