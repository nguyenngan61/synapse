...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const channelsModalOverlay = document.getElementById(
    "channels-modal-overlay"
  );
  const closeChannelsModalBtn = document.getElementById(
    "close-channels-modal-btn"
  );
  ...
  showChannelsBtn.addEventListener("click", () => {
    channelsModalOverlay.classList.remove("hidden");
  });
  closeChannelsModalBtn.addEventListener("click", () => {
    channelsModalOverlay.classList.add("hidden");
  });
  channelsModalOverlay.addEventListener("click", (e) => {
    if (e.target === channelsModalOverlay) {
      channelsModalOverlay.classList.add("hidden");
    }
  });
      ...
      .forEach((u) => {
        ...
        if (u.username !== user) {
          userItem.addEventListener("click", () => {
            ...
            socket.emit("fetch private history", {
              user1: user,
              user2: u.username,
            });
    ...
    try {
      const response = await fetch(
        `/search?term=${encodeURIComponent(
          searchTerm
        )}&room=${encodeURIComponent(currentChat.name)}`
      );
  ...
  document.addEventListener("click", () => {
    if (emojiPicker && emojiPicker.classList.contains("visible")) {
  ...
  const addMessage = (message, isPrivate = false) => {
    ...
    const lastMessageTime = lastMessageInfo.timestamp
      ? new Date(lastMessageInfo.timestamp)
      : null;
    const senderName = isPrivate ? message.fromUser : message.user;
    const isConsecutive =
      senderName === lastMessageInfo.user &&
      !message.parentMessage &&
      lastMessageTime &&
      messageTime - lastMessageTime < CONSECUTIVE_MESSAGE_TIMEOUT;
    ...
    if (message.parentMessage && message.parentMessage.user) {
      ...
      const contentPreview = message.parentMessage.content.startsWith("http")
        ? "[Hình ảnh]"
        : message.parentMessage.content.length > 50
    ...
    if (isToday) {
      timeElement.textContent = timestamp.toLocaleTimeString("vi-VN", {
        hour: "2-digit",
        minute: "2-digit",
      });
    } else {
      timeElement.textContent = timestamp.toLocaleString("vi-VN", {
        day: "2-digit",
        month: "2-digit",
        hour: "2-digit",
        minute: "2-digit",
    ...
    replyBtn.onclick = () => {
      ...
      replyingBanner.innerHTML = `<div id="replying-to-banner-content"><strong>Replying to ${senderName}:</strong> ${
        message.type === "image" ? "[Hình ảnh]" : message.content
      }</div><button id="cancel-reply-btn">&times;</button>`;
    ...
    if (senderName === user) {
      ...
      deleteBtn.onclick = () => {
        if (confirm("Bạn có chắc muốn xóa tin nhắn này?")) {
          isPrivate
            ? socket.emit("delete private message", message._id)
      ...
      if (message.type === "text") {
        ...
        editBtn.onclick = () => {
          ...
          editInput.type = "text";
          editInput.className = "message-content-input";
  ...
  form.addEventListener("submit", function (e) {
    ...
    const parentMessageData = replyingToMessage
      ? {
          _id: replyingToMessage._id,
          user: replyingToMessage.user || replyingToMessage.fromUser,
  ...
  socket.on("load old messages", (oldMessages) => {
    messages.innerHTML = "";
    lastMessageInfo = { user: null, timestamp: null };
    oldMessages.forEach((msg) => addMessage(msg, false));
  });
  socket.on("private history loaded", (history) => {
    messages.innerHTML = "";
    lastMessageInfo = { user: null, timestamp: null };
  ...
  socket.on("chat message", (msg) => {
    if (currentChat.type === "channel" && currentChat.name === msg.room) {
      ...
      if (document.hidden && msg.user !== user) {
        notificationCount++;
  ...
  socket.on("receive private message", (msg) => {
    ...
    const isInPrivateChatWithSender =
      currentChat.type === "private" && currentChat.name === chatPartner;
  ...
  socket.on("private message edited", (updatedMessage) => {
    const messageElement = document.getElementById(updatedMessage._id);
    if (messageElement) {
      const contentElement = messageElement.querySelector(".message-content");
      if (contentElement) {
  ...
  input.addEventListener("input", () => {
    socket.emit("typing", { user, room: currentChat.name });
  ...
  function formatTimeAgo(dateString) {
    ...
    if (minutes < 60) return `offline ${minutes} minutes ago`;
    if (hours < 24) return `offline ${hours} hours ago`;
    return `offline ${days} days ago`;
  }
  socket.on("update user list", (users) => {
    currentUsers = users.filter((u) => u != null);
    updateUserList();
  });
  setInterval(() => {
    if (usersModalOverlay && !usersModalOverlay.classList.contains("hidden")) {
      updateUserList();
    }
  }, 60000);
});
