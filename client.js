...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUserList() {
        ...
        currentUsers
            .sort((a, b) => {
                if (a.username === user) return -1; if (b.username === user) return 1;
                if (a.status === 'online' && b.status !== 'online') return -1; if (b.status === 'online' && a.status !== 'online') return 1;
                return 0;
            })
            .forEach(u => {
                if (!u || !u.username) return;
                const userItem = document.createElement('li'); userItem.className = u.status;
                ...
                const username = document.createElement('span'); username.className = 'username'; username.textContent = u.username;
                userItem.appendChild(statusDot); userItem.appendChild(username);
    ...
    searchForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            messages.innerHTML = ''; const resultsBanner = document.createElement('div');
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        try {
            ...
            const data = await response.json(); let messageData;
    ...
    const addMessage = (message, isPrivate = false) => {
        const item = document.createElement('li'); item.id = message._id;
        ...
        if (!isPrivate && message.parentMessage && message.parentMessage.user) { const quote = document.createElement('div'); quote.className = 'parent-message-quote'; const quoteUser = document.createElement('strong'); quoteUser.textContent = message.parentMessage.user; const quoteContent = document.createElement('span'); const contentPreview = message.parentMessage.content.startsWith('http') ? '[Hình ảnh]' : (message.parentMessage.content.length > 50 ? message.parentMessage.content.substring(0, 50) + '...' : message.parentMessage.content); quoteContent.textContent = `: ${contentPreview}`; quote.appendChild(quoteUser); quote.appendChild(quoteContent); item.appendChild(quote); }
        const sender = document.createElement('strong'); sender.textContent = senderName; item.appendChild(sender);
        if (message.type === 'image') {
            const img = document.createElement('img'); img.src = message.content; img.className = 'chat-image'; item.appendChild(img);
        } else {
            const content = document.createElement('span'); content.className = 'message-content'; content.textContent = message.content; item.appendChild(content);
        }
        if (senderName === user) { item.classList.add('my-message'); } else { item.classList.add('other-message'); }
        const buttonsWrapper = document.createElement('div'); buttonsWrapper.className = 'message-buttons';
        if (!isPrivate) {
            const replyBtn = document.createElement('button'); replyBtn.className = 'reply-btn'; replyBtn.textContent = 'Reply';
            replyBtn.onclick = () => { replyingToMessage = message; replyingBanner.innerHTML = `<div id="replying-to-banner-content"><strong>Replying to ${message.user}:</strong> ${message.type === 'image' ? '[Hình ảnh]' : message.content}</div><button id="cancel-reply-btn">&times;</button>`; replyingBanner.classList.remove('hidden'); document.getElementById('cancel-reply-btn').addEventListener('click', () => { replyingToMessage = null; replyingBanner.classList.add('hidden'); }); input.focus(); };
                ...
                editBtn.onclick = () => {
                    ...
                    editInput.onkeydown = (e) => {
                        if (e.key === 'Enter') {
                            ...
                            if (newContent && newContent !== currentContent) { socket.emit('edit message', { messageId: message._id, newContent }); }
    ...
    socket.on('receive private message', (msg) => {
        ...
        if (isInPrivateChatWithSender) {
            ...
            updateUserList();
            if (document.hidden) {
                notificationCount++; document.title = `(${notificationCount}) New PM from ${chatPartner}!`;
                notificationSound.currentTime = 0; notificationSound.play().catch(e => {});
            }
        }
    });
